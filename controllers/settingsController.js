...
var settingsController = {
  ...
  getDetails: function (req, res, next) {
    connection.query(
      ...
      (err, institutions, fields) => {
        connection.query(
          "SELECT * FROM institutions_categories where IC_Status = 1 ",
          [],
          (err, Categories, fields) => {
            connection.query(
              `SELECT ISC.* , 1 as 'ISC_Existe' FROM institution_specialities as InstSp
              ...
              [
                req.session.Institution_ID,
                institutions[0].Institution_Category_ID,
              ],
              (err, Specialities, fields) => {
                institutions[0]["Specialities"] = Specialities;
                institutions[0]["Categories"] = Categories;
                institutions[0]["params"] = [
                  req.session.Institution_ID,
                  institutions[0].Institution_Category_ID,
                ];
                institutions[0]["Category_ID"] =
                  institutions[0].Institution_Category_ID;
            ...
            connection.query(
              ...
              (err, levels, fields) => {
                connection.query(
                  ...
                  [req.session.Institution_ID],
                  (err, allSubjects, fields) => {
                    connection.query(
                      "SELECT ls.*,s.Subject_Label,s.Subject_Color FROM levelsubjects ls INNER JOIN subjects s ON s.Subject_ID = ls.Subject_ID WHERE ls.AY_ID = ? AND ls.LS_Status=1 ",
                      [academic[0].AY_ID],
                      (err, subjects, fields) => {
  ...
  getAllSubjects: function (req, res, next) {
    connection.query(
      "SELECT Subject_ID , Subject_Label as id , Subject_Label as text, Subject_Color , Subject_Color as color FROM subjects where Institution_ID = ? AND Subject_Status = 1 ",
      [req.session.Institution_ID],
      (err, subjects, fields) => {
        res.json({
          subjects: subjects,
            ...
            connection.query(
              ...
              (err, levels, fields) => {
                connection.query(
                  ...
                  (err, costs, fields) => {
                    connection.query(
                      "SELECT e.* FROM expenses e WHERE e.AY_ID= ? AND e.Expense_Status = 1 ",
                      [academic[0].AY_ID],
                      (err, expenses, fields) => {
  ...
  getSchedules: function (req, res, next) {
    connection.query(
      ...
      (err, academic, fields) => {
        connection.query(
          "SELECT * FROM institution_schedules where IS_Status <> 0 AND	Institution_ID = ? AND AY_ID = ? ",
          [req.session.Institution_ID, academic[0].AY_ID],
          (err, Schedules, fields) => {
            connection.query(
              "SELECT * FROM institution_breaks_schedule where IBS_Status = 1 AND	Institution_ID = ? AND AY_ID = ? ",
              [req.session.Institution_ID, academic[0].AY_ID],
              (err, ScheduleBreaks, fields) => {
  ...
  updateDetails: async function (req, res, next) {
    connection.query(
      "UPDATE `institutions` SET `Institution_Name`=?,`Institution_Logo`=?,`Institution_Email`=?,`Institution_Phone`=?,`Institution_Adress`=? , `Institution_Category_ID` = ?  WHERE Institution_ID=?",
      [
        req.body.Institution_Name,
        req.body.Institution_Logo,
        req.body.Institution_Email,
        req.body.Institution_Phone,
        req.body.Institution_Adress,
        req.body.Institution_Category_id,
        req.session.Institution_ID,
      ],
      async (err, institutions, fields) => {
        if (err) {
          ...
          connection.query(
            "UPDATE `institutions_website_info` SET `Institution_Category_ID` = ?  WHERE Institution_ID=?",
            [req.body.Institution_Category_id, req.session.Institution_ID],
              ...
              if (err) {
                ...
                connection.query(
                  ...
                  async (err, academic, fields) => {
                    ...
                    await commonModel
                      .removeInstitutionSpecialities(
                        req.session.Institution_ID,
                        academic[0].AY_ID
                      )
                      .then(async () => {
                        // Save new Specialities
                        req.body.Institution_SubCategories.map(
                          async (speciality) => {
                            var saveInstitutionSpecialitiesRes =
                              await commonModel.saveInstitutionSpecialities(
                                speciality,
                                req.session.Institution_ID,
                                academic[0].AY_ID
                              );
  ...
  updateAY: function (req, res, next) {
    connection.query(
      "UPDATE `academicyear` SET `AY_Label`=?,`AY_Satrtdate`=? , `AY_EndDate`=? WHERE Institution_ID=?",
      [
        req.body.AY_Label,
        req.body.AY_Satrtdate,
        req.body.AY_EndDate,
        req.session.Institution_ID,
      ],
      (err, subjects, fields) => {
            ...
            if (err) {
              ...
              for (var i = req.body.expenses.length - 1; i >= 0; i--) {
                if (
                  !expenses.some(
                    (level) =>
                      level.Expense_ID === parseInt(req.body.expenses[i].id)
                  )
                ) {
                  connection.query(
                    "INSERT INTO `levels`(`Level_Label`, `AY_ID`) VALUES (?,?)",
                    [req.body.levels[i].label, academic[0].AY_ID]
                  );
                } else
                  connection.query(
                    "UPDATE `levels` SET `Level_Label`=? WHERE Level_ID=? AND AY_ID=?",
                    [
                      req.body.levels[i].label,
                      req.body.levels[i].id,
                      academic[0].AY_ID,
                    ]
                  );
              }
    ...
    connection.query(
      ...
      async (err, academic, fields) => {
        for (var j = 0; j < req.body.subjects.length; j++) {
          ...
          connection.query(
            "update levelsubjects set LS_Status = 0 WHERE  Level_ID = ? and AY_ID = ? ",
            [req.body.subjects[j].level_id, academic[0].AY_ID],
            async (err, academic, fields) => {}
          );
          ...
          for (var s = 0; s < req.body.subjects[j].subject.length; s++) {
            ...
            if (req.body.subjects[j].subject[s].id == -1) {
              var subjectID = await settingModel.saveSubjects(
                req.body.subjects[j].subject[s].text,
                req.session.Institution_ID
              );
              var levelsubjectResult = await settingModel.saveLevelsSubjects(
                req.body.subjects[j].level_id,
                subjectID,
                academic[0].AY_ID
              );
            } else {
              var findLevelsSubjects = await settingModel.findLevelsSubjects(
                req.body.subjects[j].level_id,
                req.body.subjects[j].subject[s].id,
                academic[0].AY_ID
              );

              if (findLevelsSubjects[0].LS_Count == 0) {
                var saveLevelsSubjectsCounter =
                  await settingModel.saveLevelsSubjects(
                    req.body.subjects[j].level_id,
                    req.body.subjects[j].subject[s].id,
                    academic[0].AY_ID
                  );
                ...
                connection.query(
                  "update levelsubjects set LS_Status = 1 WHERE  Level_ID = ? and Subject_ID = ? and AY_ID = ? ",
                  [
                    req.body.subjects[j].level_id,
                    req.body.subjects[j].subject[s].id,
                    academic[0].AY_ID,
            ...
            if (err) {
              ...
              for (var i = req.body.expenses.length - 1; i >= 0; i--) {
                if (
                  !expenses.some(
                    (expense) =>
                      expense.Expense_ID === parseInt(req.body.expenses[i].id)
                  )
                ) {
                  connection.query(
                    "INSERT INTO `expenses`(`Expense_Label`,Expense_PaymentMethod,`AY_ID`,Expense_Color) VALUES (?,?,?,?)",
                    [
                      req.body.expenses[i].label,
                      req.body.expenses[i].method,
                      academic[0].AY_ID,
                      getRandomColor(),
                    ]
                  );
                  ...
                  connection.query(
                    "UPDATE `expenses` SET `Expense_Label`=?,Expense_PaymentMethod=? WHERE Expense_ID=? AND AY_ID=?",
                    [
                      req.body.expenses[i].label,
                      req.body.expenses[i].method,
                      req.body.expenses[i].id,
                      academic[0].AY_ID,
                    ]
                  );
              }
            ...
            if (err) {
              ...
              for (var i = req.body.classes.length - 1; i >= 0; i--) {
                if (
                  !classes.some(
                    (classe) =>
                      classe.Level_ID === parseInt(req.body.classes[i].level) &&
                      classe.Classe_ID === parseInt(req.body.classes[i].id)
                  )
                ) {
                  connection.query(
                    "INSERT INTO `classes`(Classe_Label,`Level_ID`, `AY_ID`) VALUES (?,?,?)",
                    [
                      req.body.classes[i].label,
                      req.body.classes[i].level,
                      academic[0].AY_ID,
                    ]
                  );
                } else {
                  connection.query(
                    "UPDATE `classes` SET `Classe_Label`=? WHERE Classe_ID=? AND Level_ID=? AND AY_ID=?",
                    [
                      req.body.classes[i].label,
                      req.body.classes[i].id,
                      req.body.classes[i].level,
                      academic[0].AY_ID,
                    ]
                  );
            ...
            if (err) {
              ...
              for (var i = req.body.costs.length - 1; i >= 0; i--) {
                if (
                  !costs.some(
                    (cost) =>
                      cost.Level_ID === parseInt(req.body.costs[i].level) &&
                      cost.Expense_ID === parseInt(req.body.costs[i].id)
                  )
                ) {
                  connection.query(
                    "INSERT INTO `levelexpenses`(Expense_ID,`Level_ID`,`Expense_Cost`,`AY_ID`) VALUES (?,?,?,?)",
                    [
                      req.body.costs[i].id,
                      req.body.costs[i].level,
                      req.body.costs[i].price,
                      academic[0].AY_ID,
                    ]
                  );
                } else
                  connection.query(
                    "UPDATE `levelexpenses` SET `Expense_Cost`=? WHERE Expense_ID=? AND Level_ID=? AND AY_ID=?",
                    [
                      req.body.costs[i].price,
                      req.body.costs[i].id,
                      req.body.costs[i].level,
    ...
    connection.query(
      ...
      async (err, academic, fields) => {
        if (err) {
          ...
          connection.query(
            "UPDATE institution_breaks_schedule SET IBS_Status = 0 WHERE Institution_ID = ? ",
            [req.session.Institution_ID],
            async (err, institution_breaks_schedule, fields) => {
              connection.query(
                "UPDATE institution_schedules SET IS_Status = 0 WHERE Institution_ID = ? ",
                [req.session.Institution_ID],
                async (err, institution_breaks_schedule, fields) => {
                  // Save Schedules
                  req.body.schedules.map(async (schedule) => {
                    var Schedules = await commonModel.saveInstitutionSchedules(
                      schedule["day_id"],
                      schedule["status"],
