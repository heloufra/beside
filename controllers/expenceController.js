...
var employeeController = {
                      ...
                      async (err, institutions, fields) => {
                        connection.query(
                          ...
                          async (err, academic, fields) => {
                            ...
                            const allEmployees =
                              await commonModel.getAllEmployees(
                                req.session.Institution_ID
                              );
  ...
  getAllExpences: async function (req, res, next) {
    const expences = await expenceModal.getAllExpences(
      req.session.Institution_ID
    );
    const employees = await commonModel.getAllEmployees(
      req.session.Institution_ID
    );
    const functionalities = await commonModel.getAllFunctionalities(
      req.session.Institution_ID
    );
  ...
  saveExpence: async function (req, res, next) {
    ...
    connection.query(
      ...
      async (err, academic, fields) => {
        connection.query(
          "insert into internal_expences(Expence_Name,Expence_Image,Expence_Color,Expence_Type,Expence_Periode,Expence_For,Expence_For_Functionalities,Institution_ID,AY_ID) values(?,?,?,?,?,?,?,?,?)",
          [
            req.body.expence_label,
            req.body.expence_image,
            commonModel.getRandomColor(),
            req.body.expence_type,
            req.body.expence_period,
            req.body.expence_for,
            req.body.expence_for_functionality,
            req.session.Institution_ID,
            academic[0].AY_ID,
          ],
          async (err, expence, fields) => {
            if (err) {
              console.log(err);
              res.json({
                errors: [
              ...
              //const expences = await expenceModal.getExpence(req.body.expence_label,req.session.Institution_ID,academic[0].AY_ID);
              //Expence_ID = expences.expences[0].Expence_ID;
              Expence_ID = expence.insertId;

              if (req.body.expence_for == "Employees") {
                const employeesExpenceCost =
                  await expenceModal.saveEmployeesExpenceCost(
                    Expence_ID,
                    req.body.employees_list
                  );
              } else {
                const institutionExpenceCost =
                  await expenceModal.saveInstitutionExpenceCost(
                    Expence_ID,
                    req.body.expence_cost
                  );
              }
  ...
  deleteExpence: function (req, res, next) {
    connection.query(
      "UPDATE internal_expences SET Expence_Status = 0 WHERE Expence_ID = ? Limit 1 ",
      [req.body.id],
      (err, expences, fields) => {
        if (err) {
          console.log(err);
    ...
    connection.query(
      ...
      async (err, academic, fields) => {
        if (err) {
          ...
          const institutionExpence =
            await expenceModal.updateInstitutionExpence(
              req.body.expence_label,
              req.body.expence_image,
              Expence_ID
            );
          ...
          if (req.body.expence_for == "Employees") {
            ...
            await expenceModal.updateInstitutionExpenceCost(
              req.body.expence_cost,
              Expence_ID
            );
          }
    ...
    connection.query(
      ...
      async (err, academic, fields) => {
        if (err) {
          ...
          if (req.body.expence_for == "Employees") {
            await expenceModal.executeEmployeesPayement(
              Expence_ID,
              req.body.employees_list,
              req.body.paid_month
            );
          } else {
            await expenceModal.executeInstitutionPayement(
              req.body.expence_cost,
              Expence_ID,
              req.body.paid_month
            );
