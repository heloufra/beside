...
var teacherController = {
  ...
  teacherView: function(req, res, next) {
    ...
    connection.query("SELECT * FROM users WHERE User_ID = ? LIMIT 1", [req.userId], (err, user, fields) => {
      ...
      connection.query("SELECT institutions.* FROM users INNER JOIN institutionsusers ON institutionsusers.User_ID = users.User_ID INNER JOIN institutions ON institutionsusers.Institution_ID = institutions.Institution_ID WHERE users.User_ID = ? AND institutionsusers.User_Role='Admin'", [req.userId], (err, accounts, fields) => {
        ...
        connection.query("SELECT users.*,institutionsusers.User_Role as role FROM users INNER JOIN institutionsusers ON institutionsusers.User_ID = users.User_ID INNER JOIN institutions ON institutionsusers.Institution_ID = institutions.Institution_ID WHERE institutions.Institution_ID = ? AND users.User_ID = ? AND institutionsusers.IU_Status = 1 And users.User_Status = 1 ", [req.Institution_ID,req.userId], (err, users, fields) => {  

          connection.query("SELECT * FROM `institutions` WHERE `Institution_ID` = ? And Institution_Status = 1 LIMIT 1", [req.Institution_ID], (err, institutions, fields) => {
            ...
            connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, academic, fields) => {
              ...
              connection.query("SELECT * FROM `classes` WHERE AY_ID = ? And Classe_Status = 1 ", [academic[0].AY_ID], (err, classes, fields) => {
                ...
                connection.query("SELECT * FROM `levels` WHERE AY_ID = ? And Level_Status = 1 ", [academic[0].AY_ID], (err, levels, fields) => {
                  ...
                  connection.query("SELECT subjects.*,levelsubjects.Level_ID FROM subjects INNER JOIN levelsubjects ON levelsubjects.Subject_ID = subjects.Subject_ID WHERE levelsubjects.AY_ID = ? And subjects.Subject_Status = 1 And levelsubjects.LS_Status = 1 GROUP by subjects.Subject_Label", [academic[0].AY_ID], (err, subjects, fields) => {
            ...
            connection.query("SELECT * FROM subjects WHERE Subject_Status = 1 and Institution_ID = ? ", [req.Institution_ID] ,async (err, allsubjects, fields) => {
              ...
              connection.query("SELECT User_Name , User_Email , User_Phone , User_Image , User_Gender , User_Address , User_Birthdate from users where  User_ID = ? ", [req.query.id] ,async (err, teacher, fields) => {
                  ...
                  for (var i = uniquecls.length - 1; i >= 0; i--) {
                    temp = await teacherModel.getAllClasses(uniquecls[i],academic[0].AY_ID);
                    if (temp){
                  ...
                  if(salary.length > 0 ){
                    ...
                    teacher[0]["User_Salary_EEC_ID"] = salary[0].EEC_ID;
                  }else{
  ...
  getClasses: function(req, res, next) {
      connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, academic, fields) => {   
          connection.query("SELECT classes.*,levels.Level_Label FROM levelsubjects INNER JOIN classes ON classes.Level_ID = levelsubjects.Level_ID INNER JOIN levels ON levels.Level_ID = levelsubjects.Level_ID WHERE levelsubjects.Subject_ID = ? AND levelsubjects.AY_ID = ? And classes.Classe_Status = 1 ", [req.query.subject_id,academic[0].AY_ID], (err, classes, fields) => {
              res.json({
                    classes:classes,
                  });
           })
  ...
  getAllteachers: async function(req, res, next) {
      ...
      connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], async (err, academic, fields) => {  
        ...
        connection.query("SELECT users.User_ID,users.User_Name,users.User_Image,users.User_Email,users.User_Phone,users.User_Gender,users.User_Birthdate,users.User_Address FROM `institutionsusers` INNER JOIN users ON users.User_ID = institutionsusers.User_ID WHERE institutionsusers.`Institution_ID` = ? AND institutionsusers.User_Role = 'Teacher' AND institutionsusers.IU_Status = 1 And users.User_Status = 1 Order By users.User_ID Desc ", [req.Institution_ID], async (err, teachers, fields) => {

          for (var i = teachers.length - 1; i >= 0; i--) { 

            var classes = await teacherModel.findClasses(teachers[i].User_ID,academic[0].AY_ID);
            var subjects = await teacherModel.findSubjects(teachers[i].User_ID,academic[0].AY_ID);
            teachersAbsenceDelay = await teacherModel.getTeachersAbsenceDelay(teachers[i].User_ID,req.Institution_ID,academic[0].AY_ID);
            teachers[i].teachersAbsenceDelay = teachersAbsenceDelay;
            const salary = await commonModel.getUserSalary(teachers[i].User_ID,req.Institution_ID);
 ...
 saveTeacher: async function(req, res, next) {
      ...
      if(Object.keys(user_error).length === 0 && user_error.constructor === Object) {
        ...
        connection.query("SELECT * from institutions where`Institution_ID` = ? LIMIT 1 ", [req.Institution_ID], async (err, academic, fields) => {
              ...
              else
              {
                  ...
                  Expence_ID = await commonModel.getCurrentExpencesSalariesID(req.Institution_ID);
                  await commonModel.addUserSalary(Expence_ID,userId,req.body.teacher_salary);
             ...
             if (err) {
              ...
              } else 
              {
                ...
                if (req.body.subjects)
                {
                  ...
                  for (var i = req.body.subjects.length - 1; i >= 0; i--) {
                    for (var j =  req.body.subjects[i].classes.length - 1; j >= 0; j--) {
                      var teacher = await teacherModel.findSubTeacher(req.body.id,req.body.subjects[i].subject,req.body.subjects[i].classes[j],academic[0].AY_ID)
                      if (teacher.length === 0)
                      {
                ...
                if(!req.body.teacher_salary_eec_id || req.body.teacher_salary_eec_id == -1 ){
                  Expence_ID = await commonModel.getCurrentExpencesSalariesID(req.Institution_ID);
                  await commonModel.addUserSalary(Expence_ID,req.body.id,req.body.teacher_salary);
                }else{
                  await commonModel.updateUserSalary(req.body.teacher_salary,req.body.teacher_salary_eec_id);
                }
            ...
            for (var i = rows.length - 1; i >= 0; i--) {
              ...
              if (user.length > 0)
              {
                if (!user.some(user => user.role === 'Teacher'))
                {
                  ...
                  user = await teacherModel.updateTeacher(rows[i][0],rows[i][1],rows[i][5],rows[i][3],rows[i][4],rows[i][6],rows[i][2],userId);
                } else {
               ...
               {
                 //first_name,last_name,email,birthdate,  phone_number,teacher_address,teacher_gender
                  user = await teacherModel.saveTeacher(rows[i][0],rows[i][1],rows[i][5],rows[i][3],rows[i][4],rows[i][6],rows[i][2],req.Institution_ID);
                  console.log("user import ==>",user);
                  console.log("user user.Institution_Name ==>",user.Institution_Name);
