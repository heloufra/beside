...
var websiteController = {
                  ...
                  connection.query("SELECT * FROM `expenses` WHERE AY_ID = ? AND Expense_Status=1", [academic[0].AY_ID], (err, expenses, fields) => {
                    connection.query("SELECT ls.*,s.Subject_Label FROM levelsubjects ls INNER JOIN subjects s ON s.Subject_ID = ls.Subject_ID WHERE ls.AY_ID = ? AND ls.LS_Status=1 AND Institution_ID = ? ", [academic[0].AY_ID,req.Institution_ID], (err, subjects, fields) => {
                      connection.query("SELECT ls.*,e.Expense_Label,e.Expense_PaymentMethod FROM levelexpenses ls INNER JOIN expenses e ON e.Expense_ID = ls.Expense_ID WHERE ls.AY_ID= ? AND ls.LE_Status=1", [academic[0].AY_ID], (err, costs, fields) => {
                        res.render('website', { title: 'Website' , user: user[0], institution:institutions[0], classes:classes,levels:levels,accounts,users,expenses,academic:academic[0],subjects,costs,role:req.role});
                      })
                    })
                  }) 
                })
  ...
  getDetails:function(req,res, next) {
    connection.query("SELECT * FROM `institutions_website_info` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, institutions, fields) => {
  ...
  getAmenties:function(req,res, next) {
        connection.query("SELECT * FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, academic, fields) => {
          connection.query(`SELECT a.* , 1 as 'Amenty_Selected' FROM amenties a inner join institution_amenties ai on(a.Amenty_ID = ai.Amenty_ID)  
                          ...
                          WHERE a.Amenty_Status = 1 and ai.IA_Status = 1 and ai.Institution_ID = ? AND ai.AY_ID = ?)`,
            [req.Institution_ID,academic[0].AY_ID,req.Institution_ID,academic[0].AY_ID], (err, Amenties, fields) => {
                  res.json({Amenties});
            });
      });
  },
  getDisplaySettings:function(req,res, next) {
        ...
        connection.query(`SELECT mtd.* , 1 as 'MTD_Selected' FROM modules_to_display mtd inner join institution_modules_to_display_settings imtds
            ...
            [req.Institution_ID,academic[0].AY_ID,req.Institution_ID,academic[0].AY_ID], (err, DisplaySettings, fields) => {
  ...
  getInstitutionApiInfo: async function (req, res, next) {
    ...
    let queryInstitution = "SELECT IWI.* , I.Institution_Logo FROM institutions_website_info as IWI  INNER JOIN institutions I ON(I.Institution_ID = IWI.Institution_ID) ";
    ...
    connection.query(queryInstitution, [req.query.name], async (err, institutions, fields) => {
        
      if (err) {
        console.log(err);
        res.json({ "Error": err });
      } else {
          await websiteModel.getInstitutionApiInfo(institutions).then((data) => {
            res.json({data});
          });
      }
  ...
  updateDetails: async function(req, res, next) {
   connection.query('UPDATE `institutions_website_info` SET `Institution_Name`=?,`Institution_Email`=?,`Institution_Phone`=?,`Institution_Adress`=? ,`Institution_About`=? , `Institution_Link`= ? , `Institution_Latitude`= ? , `Institution_Longitude`= ? , `Institution_Location`= ?   WHERE Institution_ID=?', [req.body.Institution_Name,req.body.Institution_Email,req.body.Institution_Phone,req.body.Institution_Adress,req.body.Institution_About,req.body.Institution_Website,req.body.Institution_Latitude,req.body.Institution_Longitude,req.body.Institution_Location,req.Institution_ID], async (err, institutions_website_info, fields) => {
      if (err)
      {
  ...
  updateAmenties: async function(req,res){
    connection.query("SELECT * FROM academicyear WHERE Institution_ID = ? LIMIT 1", [req.Institution_ID], async (err, academic, fields) => {
         if (err) {
            ...
            connection.query("UPDATE institution_amenties SET IA_Status = 0 WHERE Institution_ID = ? ",[req.Institution_ID], async (err, institution_amenties, fields) => {
              // Save Amenties
              req.body.amenties.map( async (amenty) => {
                  var Result = await commonModel.saveInstitutionAmenties(amenty,req.Institution_ID,academic[0].AY_ID);
              });
              res.json({updated:true});
            });
        }
  ...
  updateDisplaySettings: async function(req,res){
    connection.query("SELECT * FROM academicyear WHERE Institution_ID = ? LIMIT 1", [req.Institution_ID], async (err, academic, fields) => {
         if (err) {
            ...
            connection.query("UPDATE institution_modules_to_display_settings SET IMTDS_Status = 0 WHERE Institution_ID = ? ",[req.Institution_ID], async (err, institution_modules_to_display_settings, fields) => {
              // Save Display Settings
              if (req.body.display_Settings && req.body.display_Settings.length > 0 ) {
                req.body.display_Settings.map( async (mtd_ID) => {
                    var Result = await commonModel.saveInstitutionDisplaySettings(mtd_ID,req.Institution_ID,academic[0].AY_ID);
                });                
              }
              res.json({updated:true});
            });
