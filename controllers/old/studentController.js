...
var querySubstudent = "SELECT MONTHNAME(studentsubscribtion.Subscription_EndDate) as 'Subscription_EndDate_Month' , studentsubscribtion.Subscription_EndDate ,   studentsubscribtion.SS_Status ,  levelexpenses.Level_ID,levelexpenses.Expense_Cost,expenses.Expense_Label,expenses.Expense_PaymentMethod,studentsubscribtion.SS_ID,studentsubscribtion.Subscription_StartDate FROM students INNER JOIN studentsubscribtion ON students.Student_ID = studentsubscribtion.Student_ID INNER JOIN levelexpenses ON levelexpenses.LE_ID = studentsubscribtion.LE_ID INNER JOIN expenses ON expenses.Expense_ID = levelexpenses.Expense_ID INNER JOIN academicyear ON academicyear.Institution_ID = students.Institution_ID WHERE students.Student_ID = ? AND students.Institution_ID = ? ";
...
var studentPayment = "SELECT MONTHNAME(studentsubscribtion.Subscription_EndDate) as 'Subscription_EndDate_Month' , studentsubscribtion.Subscription_EndDate ,   studentsubscribtion.SS_Status , studentsubscribtion.SS_ID,expenses.Expense_PaymentMethod,expenses.Expense_Label,studentspayments.*,levelexpenses.Expense_Cost FROM `studentsubscribtion` INNER JOIN studentspayments ON studentspayments.SS_ID = studentsubscribtion.SS_ID INNER JOIN levelexpenses ON levelexpenses.LE_ID = studentsubscribtion.LE_ID INNER JOIN expenses ON expenses.Expense_ID = levelexpenses.Expense_ID WHERE studentsubscribtion.Student_ID = ? ";
var adddate = 1;
var classeID;
...
var studentController = {
            ...
            connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, academic, fields) => {
              connection.query("SELECT * FROM `classes` WHERE AY_ID = ?", [academic[0].AY_ID], (err, classes, fields) => {
                connection.query("SELECT * FROM `levels` WHERE AY_ID = ?", [academic[0].AY_ID],async (err, levels, fields) => {
                  if (req.role === 'Teacher')
                      classes = await teacherModel.findClasses(req.userId,academic[0].AY_ID);
  ...
  getAllStudents: async function(req, res, next) {
    ...
    connection.query("SELECT * FROM `institutions` WHERE `Institution_ID` = ? LIMIT 1", [req.userId], async (err, institutions, fields) => {
      connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], async (err, academic, fields) => {
        connection.query(queryAllSub, [academic[0].AY_ID], async (err, subscription, fields) => {
  ...
  saveStudent: function(req, res, next) {
     ...
     connection.query("SELECT Count(*) as 'Email_Count' FROM `students` WHERE `Student_Email` = ? AND Student_Status = 1 AND Institution_ID = ? limit 1  ", [req.body.student_email , req.Institution_ID ], (err, studentEmail, fields) => {
          ...
          connection.query("SELECT Count(*) as 'Code_Count' FROM `students` WHERE `Student_Code` = ? AND Student_Status = 1  AND Institution_ID = ? limit 1 ", [req.body.student_code , req.Institution_ID ], (err, studentCode, fields) => {
             ...
             (Object.keys(student_error).length === 0 && student_error.constructor === Object) ) // user.length === 0
          {
             connection.query("SELECT * FROM `institutions` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, institutions, fields) => {
              connection.query("SELECT * FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, academic, fields) => {
                    connection.query(studentQuery, [req.body.first_name,req.body.last_name, req.body.profile_image,req.body.birthdate,req.body.student_address,req.body.phone_number,req.body.student_gender,req.body.student_email,req.Institution_ID,req.body.student_code], (err, student, fields) => {
                       if (err) {
                            console.log(err);
                          ...
                          else 
                          {
                             ...
                             for (let i = req.body.parent_name.length - 1; i >= 0; i--) {
                              ...
                              connection.query(parentQuery, [req.body.parent_name[i],parent_phone,req.body.parent_email[i],req.Institution_ID], (err, parent, fields) => {
                               if (err) {
                                    console.log(err);
                                      res.json({
                                        errors: [{
                                             ...
                                             connection.query(
                                              ...
                                              [
                                                ...
                                                if (err) {
                                                  ...
                                                  sendEmail(req.body.parent_email[i], firstName, "parent",institutions[0].Institution_Name);
                          ...
                          connection.query(
                            ...
                            if (err)
                              ...
                              sendEmail(req.body.student_email, req.body.first_name, "élève",institutions[0].Institution_Name);
  ...
  saveAbsence: async function(req, res, next) {
           connection.query("SELECT * FROM `institutions` WHERE `Institution_ID` = ? LIMIT 1", [req.userId], async (err, institutions, fields) => {
            connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], async (err, academic, fields) => {
                  connection.query(absenceQuery, [req.body.user_id,  "Student",  req.body.ad_type,  req.body.ad_fromto, req.body.ad_date, req.userId], async (err, absence, fields) => {
                     if (err) {
                          console.log(err);
                            res.json({
                              errors: [{
                        ...
                        } else 
                        {
                          ...
                          if(req.role == 'Admin'){ // change later to allow teachers 
                                ...
                                if(req.body.ad_type == 1 || req.body.ad_type == 2){
                                  notificationBody =  arrLang["fr"]["Nouvelle"]+' '+arrLang["fr"]["Notification_Absence"]+' '+arrLang["fr"]["A_Ete_Ajoutee"];
                                  module_id = 9;
                                }else{
                                  notificationBody =  arrLang["fr"]["Nouveau"]+' '+arrLang["fr"]["Notification_Retard"]+' '+arrLang["fr"]["A_Ete_Ajoute"];
                                  module_id = 10;
                                }
                                ...
                                const StudentList = await commonModel.getStudentsList(academic[0].AY_ID,req.body.user_id,req.body.ad_type);
                                      ...
                                      Receivers[r].map(async (Receiver)=>{
                                          ...
                                          if(Receiver.User_Role == "Student"){
                                            ...
                                            //await commonModel.sendPushNotification(Receivers[r].User_Expo_Token,'',strippedString);
                                              ...
                                              //add Notifications
                                              await commonModel.addNotifications(Receiver.User_ID,Receiver.User_Role,absence.insertId,module_id,Receiver.Parent_Child_ID);
                                              //send Notification
  ...
  savePayments: async function (req, res, next) {
    ...
    if (err) {
      console.log(err);
        ...
        const StudentList = await commonModel.getStudentsList(academic[0].AY_ID, req.body.studentId, 'Invoice');
          ...
          for (r = 0; r < Receivers.length; r++) {
            ...
            Receivers[r].map(async (Receiver) => {
              ...
              if (Receiver.User_Role == "Parent") {
                ...
                await commonModel.addNotifications(Receiver.User_ID, Receiver.User_Role, (Current_Month_Date_Obj.getMonth() + 1), 13, Receiver.Parent_Child_ID);
                //send Notification
                const childName = Receiver.Child_Full_Name;
  ...
  saveAttitude: async function(req, res, next) {
    //Student_ID, Attitude_Interaction, Attitude_Note, Declaredby_ID, AY_ID
     connection.query("SELECT * FROM `institutions` WHERE `Institution_ID` = ? LIMIT 1", [req.userId], async (err, institutions, fields) => {
            connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], async (err, academic, fields) => {
                  connection.query(attitudeQuery, [req.body.user_id,  req.body.at_type,  req.body.at_note, req.body.at_date, req.userId,academic[0].AY_ID], async (err, attitude, fields) => {
                     if (err) {
                          console.log(err);
                            res.json({
                              errors: [{
                        ...
                        else 
                        {
                          ...
                          //if(req.role == 'Admin'){ // change later to allow teachers 
                                ...
                                const StudentList = await commonModel.getStudentsList(academic[0].AY_ID,req.body.user_id,'Attitude');
                                      ...
                                      Receivers[r].map(async (Receiver)=>{
                                          ...
                                          if(Receiver.User_Role == "Student"){
                                            if(req.body.user_id == Receiver.User_ID ){
                                              //add Notifications
                                              await commonModel.addNotifications(Receiver.User_ID,Receiver.User_Role,attitude.insertId,8);

                                              const User = await commonModel.getUser(req.userId);
                                              //send Notification
                                              ...
                                              var notificationBody = (User.user.User_Gender == 'Female') ? arrLang["fr"]["A_Ajoutee"] : arrLang["fr"]["A_Ajoute"];
                                              notificationBody += ' '+arrLang["fr"]["Nouveau"]+' '+arrLang["fr"]["Notification_Attitude"];
                                              ...
                                              await commonModel.sendPushNotification(Receiver.User_Expo_Token,name,notificationBody);
                                              
                                            }
                                          }else{
                                              await commonModel.addNotifications(Receiver.User_ID,Receiver.User_Role,attitude.insertId,8,Receiver.Parent_Child_ID);
                                              //send Notification
                                              const childName = Receiver.Child_Full_Name ;
                                              const notificationBody = arrLang["fr"]["Nouveau"]+' '+arrLang["fr"]["Notification_Attitude"]+' '+arrLang["fr"]["A_Ete_Ajoute"];
                                              await commonModel.sendPushNotification(Receiver.User_Expo_Token,childName,notificationBody);
                                          }
  ...
  updateStudent: async function(req, res, next) {
    ...
    /**____ form_errors ______________________**/
     ...
     if(req.body.parent_phone && req.body.parent_phone.length > 0 ){
      ...
      // unique code 
      var code = await studentModel.studentUniqueCode(req.body.student_code,req.body.id,req.Institution_ID );
      // unique code 
    ...
    {
      connection.query("SELECT * FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, academic, fields) => {
        connection.query("UPDATE `students` SET `Student_FirstName`=?,`Student_LastName`=?,`Student_Image`=?,`Student_birthdate`=?,`Student_Address`=?,`Student_Phone`=?,Student_Gender=?,Student_Email=? , Student_Code = ? WHERE Student_ID = ?", [req.body.student_fname,req.body.student_lname,req.body.student_img,req.body.student_birthdat,req.body.student_address,req.body.student_phone,req.body.student_gender,req.body.student_email,req.body.student_code,req.body.id],async (err, student, fields) => {
           if (err) {
                console.log(err);
                  res.json({
                    errors: [{
                  ...
                  if (existingParent) {
                    ...
                    connection.query(spQuery, [req.body.id, existingParent.Parent_ID], (err, spresult, fields) => {
                        ...
                        if (err) {
                          ...
                          connection.query(updateParentQuery, [req.body.parent_name[i].name,parent_phone,req.body.parent_email[i].email,req.Institution_ID,existingParent.Parent_ID], (err, parent, fields) => {
                            if (err) {
                                console.log(err);
                                  res.json({
                                    errors: [{
                    ...
                    connection.query(parentQuery, [req.body.parent_name[i].name,,req.body.parent_email[i].email,req.Institution_ID], (err, parent, fields) => {
                      if (err) {
                          console.log(err);
                            res.json({
                              errors: [{
  ...
  importFile: function(req, res, next) {
    ...
    */
    ...
    connection.query("SELECT ac.* , ins.* FROM academicyear ac INNER JOIN institutions ins ON(ac.Institution_ID = ins.Institution_ID) WHERE ins.Institution_ID = ? LIMIT 1", [req.Institution_ID],async (err, academic, fields) => {
    res.json({saved:true});
    for (var i = rows.length - 1; i >= 0 ; i--) { 

            var student = await studentModel.saveStudent(rows[i][1],rows[i][2],rows[i][4],rows[i][7],rows[i][5],rows[i][3],rows[i][6],req.Institution_ID,rows[i][0]);

            sendEmail(rows[i][5],rows[i][0],"élève",academic[0].Institution_Name);
          
            if (rows[i][10]){
              var parent = await studentModel.saveParent(rows[i][10],rows[i][11],rows[i][12],rows[i][13],req.Institution_ID);
              var spresult = await studentModel.saveStudentParent(student.insertId,parent.insertId);
              sendEmail(rows[i][13],rows[i][10],"parent",academic[0].Institution_Name);
            }

            if (rows[i][14]){
               var parent = await studentModel.saveParent(rows[i][14],rows[i][15],rows[i][16],rows[i][17],req.Institution_ID);
               var spresult = await studentModel.saveStudentParent(student.insertId,parent.insertId);
               sendEmail(rows[i][17],rows[i][14],"parent",academic[0].Institution_Name);
            }
            ...
            if(rows[i][18]){
              var le_id = await studentModel.findLeByLevel(rows[i][18],academic[0].AY_ID,rows[i][8]);
              try{
            ...
            if(rows[i][19]){
              var le_id = await studentModel.findLeByLevel(rows[i][19],academic[0].AY_ID,rows[i][8]);
              try{
                  var ssresult = await studentModel.saveStudentSub(student.insertId,le_id[0].LE_ID,months[startDate.getMonth()],academic[0].AY_EndDate,academic[0].AY_ID);
              }catch(e){
                 console.log(e)
            ...
            if(rows[i][20]){
              var le_id = await studentModel.findLeByLevel(rows[i][20],academic[0].AY_ID,rows[i][8]);
              try{
                  var ssresult = await studentModel.saveStudentSub(student.insertId,le_id[0].LE_ID,months[startDate.getMonth()],academic[0].AY_EndDate,academic[0].AY_ID);
              }catch(e){
                 console.log(e)
            ...
            if(rows[i][21]){
              var le_id = await studentModel.findLeByLevel(rows[i][21],academic[0].AY_ID,rows[i][8]);
              try{
                  var ssresult = await studentModel.saveStudentSub(student.insertId,le_id[0].LE_ID,months[startDate.getMonth()],academic[0].AY_EndDate,academic[0].AY_ID);
              }catch(e){
                 console.log(e)
            ...
            var classe = await studentModel.findClasse(rows[i][9],academic[0].AY_ID);
            if(classe[0]){
