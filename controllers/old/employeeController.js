...
var employeeController = {
            ...
            connection.query("SELECT users.*,institutionsusers.User_Role as role FROM users INNER JOIN institutionsusers ON institutionsusers.User_ID = users.User_ID INNER JOIN institutions ON institutionsusers.Institution_ID = institutions.Institution_ID WHERE institutions.Institution_ID = ? AND institutionsusers.IU_Status = 1 And users.User_Status = 1 And institutionsusers.User_Role not in(?) ", [req.Institution_ID,baseRoles], async (err, users , fields) => { 
              ...
              connection.query("SELECT * FROM `institutions` WHERE `Institution_ID` = ? And Institution_Status = 1 LIMIT 1", [req.Institution_ID], async (err, institutions, fields) => {
                ...
                connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], async  (err, academic, fields) => {
                      ...
                      res.render('emplyees', { title: 'Employees' , user: user[0], institution:institutions[0],accounts,users:userRoles,role:req.role, functionalities });
  ...
  saveEmployee: async function(req, res, next) {
      ...
      if(Object.keys(user_error).length === 0 && user_error.constructor === Object) {
        ...
        connection.query("SELECT * from institutions where`Institution_ID` = ? LIMIT 1 ", [req.Institution_ID], async (err, academic, fields) => {
              ...
              var user = await employeeModel.findUser(req.body.email,req.Institution_ID);
              var userId;
              var exist = false;
              ...
              if (user.length > 0)
              {
                if (!user.some(user => user.role === 'Teacher'))
                {
                  ...
                  exist = true;
                  connection.query("UPDATE `users` SET User_Name=?,User_Image=?,User_Birthdate = ?, User_Address = ?, User_Gender=? WHERE User_ID = ?", [JSON.stringify({first_name:req.body.first_name, last_name:req.body.last_name}),req.body.profile_image, req.body.birthdate, req.body.employee_address,req.body.employee_gender,userId]);
                } else {
              ...
              else
              {
                  ...
                  userIU = await employeeModel.saveIU(req.Institution_ID,user.insertId,req.body.employee_functionality);
                  userId = user.insertId;
                  Expence_ID = await commonModel.getCurrentExpencesSalariesID(req.Institution_ID);
                  await commonModel.addUserSalary(Expence_ID,userId,req.body.employee_salary);
  ...
  deleteEmployee: function(req, res, next) {
    connection.query("UPDATE `users` SET `User_Status`=0 WHERE `User_ID` = ? AND User_Role=?", [req.body.id,req.body.role], (err, user, fields) => {
      connection.query("UPDATE `institutionsusers` SET `IU_Status`=0 WHERE `User_ID` = ? AND User_Role=?", [req.body.id,req.body.role], (err, teacher, fields) => {
  ...
  updateEmployee: async function(req, res, next) {
      ...
      if( Object.keys(user_error).length === 0 && user_error.constructor === Object) {
        ...
        connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, academic, fields) => {
          connection.query("UPDATE `users` SET User_Name=?, User_Image=?, User_Email=?,User_Birthdate=?, User_Phone=?,User_Address=? , User_Gender = ? , User_Role =? WHERE User_ID = ?", [JSON.stringify({first_name:req.body.first_name, last_name:req.body.last_name}), req.body.profile_image,req.body.email,  req.body.birthdate,  req.body.phone_number,req.body.employee_address,req.body.employee_gender,req.body.employee_functionality,req.body.id],async (err, teacher, fields) => {
             if (err) {
                  console.log(err);
                    res.json({
                      errors: [{
              ...
              {
                if(!req.body.employee_salary_eec_id || req.body.employee_salary_eec_id == -1 ){
                  Expence_ID = await commonModel.getCurrentExpencesSalariesID(req.Institution_ID);
                  await commonModel.addUserSalary(Expence_ID,req.body.id,req.body.employee_salary);
                }else{
                   await commonModel.updateUserSalary(req.body.employee_salary,req.body.employee_salary_eec_id);
                }
