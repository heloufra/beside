...
var homeworkController = {
                    ...
                    if (req.role === 'Teacher')
                      {
                        ...
                        subjects = await teacherModel.findSubjects(req.userId,academic[0].AY_ID);
                        console.log("classes=>",req.userId);
                      }
  ...
  saveHomework: async function(req, res, next) {
    connection.query("SELECT AY_ID FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], async (err, academic, fields) => {
      connection.query("SELECT teachersubjectsclasses.TSC_ID , u.* FROM `teachersubjectsclasses` INNER JOIN classes ON classes.Classe_ID = teachersubjectsclasses.Classe_ID INNER JOIN subjects ON subjects.Subject_ID = teachersubjectsclasses.Subject_ID INNER JOIN users u ON(teachersubjectsclasses.Teacher_ID = u.User_ID ) WHERE subjects.Subject_Label = ? AND classes.Classe_Label = ? AND teachersubjectsclasses.Teacher_ID = ? AND User_Status = 1 AND teachersubjectsclasses.AY_ID = ? LIMIT 1", [req.body.homework_subject,req.body.homework_classe,req.userId,academic[0].AY_ID], async (err, tsc, fields) => {
        if (tsc[0])
        {

          connection.query(homeworkQuery, [tsc[0].TSC_ID,  req.body.homework_name,  req.body.homework_description, req.body.homework_deliverydate], async (err, homework, fields) => {

            if (req.files){

              for(var f = 0 ; f < req.files.length ; f++){
                connection.query(homeworkFileQuery, [homework.insertId,req.files[f].path.replace(/\\/g, "/").replace('public',''),req.body.homework_name + '-' + req.files[f].originalname], async (err, homeworkfile, fields) => {
                 if (err) {
                      console.log(err);
                        res.json({
                          errors: [{
              ...
              const StudentList = await commonModel.getStudentsList(academic[0].AY_ID,tsc[0].TSC_ID,'Homeworks');
                ...
                for(r = 0 ; r < Receivers.length ; r++ ){
                    ...
                    Receivers[r].map(async (Receiver)=>{
                        ...
                        if(Receiver.User_Role == "Student"){
                          //add Notifications
                          await commonModel.addNotifications(Receiver.User_ID,Receiver.User_Role,homework.insertId,4);
                          //send Notification
                          ...
                          var notificationBody = (tsc[0].User_Gender == 'Female') ? arrLang["fr"]["A_Ajoutee"] : arrLang["fr"]["A_Ajoute"];
                          notificationBody += ' '+arrLang["fr"]["Nouveau"]+' '+arrLang["fr"]["Notification_Homework"];
                          ...
                          await commonModel.sendPushNotification(Receiver.User_Expo_Token,name,notificationBody);
                            ...
                            //add Notifications
                            await commonModel.addNotifications(Receiver.User_ID,Receiver.User_Role,homework.insertId,4,Receiver.Parent_Child_ID);
                            //send Notification
                            const childName = Receiver.Child_Full_Name ;
                            const notificationBody = arrLang["fr"]["Nouveau"]+' '+arrLang["fr"]["Notification_Homework"]+' '+arrLang["fr"]["A_Ete_Ajoute"];
