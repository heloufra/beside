...
var dashboardController = {
                  ...
                  async (err, institutions, fields) => {
                    connection.query(
                      ...
                      async (err, academic, fields) => {
                        ...
                        const teacherAD =
                          await commonModel.getAllTeachersAbsenceDelay(
                            req.session.Institution_ID,
                            academic[0].AY_ID
                          );
                        ...
                        const studentAD =
                          await commonModel.getAllStudentsAbsenceDelay(
                            req.session.Institution_ID,
                            academic[0].AY_ID
                          );
                        ...
                        const monthsExpenses =
                          await commonModel.getMonthsExpenses(
                            academic[0].AY_ID,
                            academic[0].AY_YearStart,
                            academic[0].AY_YearEnd,
                            academic[0].AY_Satrtdate,
                        ...
                        /********* This month Expense ***********/
                        const thisMonthsExpenses = monthsExpenses.filter(
                          (month) => month.Month_ID == date.getMonth() + 1
                        );
                            ...
                            connection.query(
                              ...
                              (err, levels, fields) => {
                                connection.query(
                                  ...
                                  (err, expenses, fields) => {
                                    connection.query(
                                      "SELECT COUNT(*) as total FROM `institutionsusers` WHERE `Institution_ID`=? AND `User_Role`='Teacher' and `IU_Status` = 1 ",
                                      [req.Institution_ID],
                                      (err, percentageT, fields) => {
                                        connection.query(
                                          "SELECT COUNT(*) as total FROM `students` WHERE `Institution_ID`=? And `student_Status` = 1 ",
                                          [req.Institution_ID],
                                          (err, percentageS, fields) => {
                                            res.render("dashboard", {
                                              ...
                                              role: req.role,
                                              teacherAdAbsences:
                                                teacherAD.Total_Absences,
                                              teacherAdRetards:
                                                teacherAD.Total_Retards,
                                              studentAdAbsences:
                                                studentAD.Total_Absences,
                                              studentAdRetards:
                                                studentAD.Total_Retards,
                                              studentsTotal:
                                                percentageS[0].total,
                                              teachersTotal:
                                                percentageT[0].total,
  ...
  getDashboardAllData: async function (req, res, next) {
    connection.query(
      "SELECT * FROM `users` WHERE `User_ID` = ? LIMIT 1",
      [req.userId],
      async (err, user, fields) => {
  ...
  getAllAbsences: function (req, res, next) {
    ...
    connection.query(
      AdTeacher,
      [req.Institution_ID],
      (err, absencesT, fields) => {
        connection.query(
          AdStudent,
          [req.Institution_ID],
          async (err, absencesS, fields) => {
            for (var i = absencesT.length - 1; i >= 0; i--) {
              var classes = await teacherModel.findClasses(
                absencesT[i].User_ID
              );
              var reportedBy = await teacherModel.findUserById(
                absencesT[i].Declaredby_ID
              );
              teachersArray.push({
                teacher: absencesT[i],
                classes: classes,
  ...
  getAllPayments: async function (req, res, next) {
    connection.query(
      "SELECT * FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1",
      [req.Institution_ID],
      async (err, academic, fields) => {
        let Month_ID = req.body.Month_ID;

        // studentMonthlySubscriptions
        await commonModel
          .getStudentMonthlySubscriptions(
            academic[0].AY_ID,
            academic[0].AY_YearStart,
            academic[0].AY_YearEnd,
            academic[0].AY_Satrtdate,
            academic[0].AY_EndDate,
            academic[0].Institution_ID,
            Month_ID
          )
          .then((studentMonthlySubscriptions) => {
            connection.query(
              PaymentsQuery,
              [req.Institution_ID],
              (err, payments, fields) => {
                connection.query(
                  "SELECT ss.*,le.Expense_Cost,e.Expense_PaymentMethod FROM studentsubscribtion ss INNER JOIN levelexpenses le ON le.LE_ID = ss.LE_ID INNER JOIN expenses e ON e.Expense_ID = le.Expense_ID INNER JOIN students s on (s.Student_ID = ss.Student_ID )  WHERE ss.AY_ID=? AND ss.SS_Status=1 and s.	Student_Status = 1 ",
                  [academic[0].AY_ID],
                  (err, studentsSub, fields) => {
                    console.log(
                      "studentMonthlySubscriptions==>",
                      studentMonthlySubscriptions
                    );
                    res.json({
                      payments,
                      studentsSub,
                      studentMonthlySubscriptions: studentMonthlySubscriptions,
                    });
                  }
                );
              }
                                            ...
                                            res.json({
                                              ...
                                              Expense_Cost:
                                                thisMonthsExpenses[0]
                                                  .Expense_Cost,
                                              TotalPay:
                                                thisMonthsExpenses[0].TotalPay,
                                              Month:
                                                thisMonthsExpenses[0].Month,
                                              Month_ID:
                                                thisMonthsExpenses[0].Month_ID,
                                              Month_Start_Date:
                                                thisMonthsExpenses[0]
                                                  .Month_Start_Date,
                                              MonthsExpenses: monthsExpenses,
