...
const queryStudentSubs = "SELECT MONTHNAME(studentsubscribtion.Subscription_EndDate) as 'Subscription_EndDate_Month' , studentsubscribtion.Subscription_EndDate ,   studentsubscribtion.SS_Status , expenses.*,levelexpenses.Expense_Cost,studentsubscribtion.Student_ID,studentsubscribtion.* FROM expenses INNER JOIN levelexpenses ON levelexpenses.Expense_ID = expenses.Expense_ID INNER JOIN studentsubscribtion ON studentsubscribtion.LE_ID = levelexpenses.LE_ID WHERE studentsubscribtion.AY_ID = ? AND ( studentsubscribtion.SS_Status = 0 OR studentsubscribtion.SS_Status = 1) AND studentsubscribtion.Student_ID=?;"
...
var financeController = {
  ...
  executePaymentBySubscription: function (req, res, next) {
    ...
    connection.query("SELECT * FROM `academicyear` WHERE `Institution_ID` = ? LIMIT 1", [req.Institution_ID], (err, academic, fields) => {
      connection.query(`SELECT ss.SS_ID From studentsubscribtion ss inner join levelexpenses le on(ss.LE_ID = le.LE_ID)  
      inner join expenses e on(e.Expense_ID = le.Expense_ID) where e.Expense_Status = 1 and le.LE_Status = 1 AND ss.SS_Status = 1 and ss.Subscription_EndDate = ? and ss.AY_ID = ? and e.Expense_ID = ? `, [ academic[0].AY_EndDate , academic[0].AY_ID , req.query.expenseId ], (err, studentSubscriptions , fields) => {

          studentSubscriptionsArray = [];

          studentSubscriptions.map((stdSub)=>{
            
            studentSubscriptionsArray.push(new Promise( (resolve, reject) => {
                    connection.query(`Insert into studentspayments(SS_ID,SP_PaidPeriod) values(?,?) `, [ stdSub.SS_ID , req.query.expensePeriod , req.query.expenseId ], (err, studentSubscriptions , fields) => {
                        if(err){
                          reject(err);
                        }else{
                          resolve(
