...
var institution_months =  ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
$domChange = false;
var start,end;
var alreadyPay = [];
var studentlevelchanged = 0 ;

var $global_student_list = '';
	  	...
	  	if(res.errors){
	  		...
	  		for (var i = res.students.length - 1; i >= 0; i--) {
	  			...
	  			dynamicListRows +='class="'+active+' sections-main-sub-container-left-card students_list"><img class="sections-main-sub-container-left-card-main-img" src="'+res.students[i].Student_Image+'" alt="card-img"><span class="sections-main-sub-container-left-card-main-img-text loading-bg-helper"></span><input name="studentId" type="hidden" value="'+res.students[i].Student_ID+'"> <div class="sections-main-sub-container-left-card-info"><p class="sections-main-sub-container-left-card-main-info">'+res.students[i].Student_FirstName+' '+res.students[i].Student_LastName+'</p><span  class="sections-main-sub-container-left-card-sub-info">'+res.students[i].Classe_Label+'</span></div>';
	  				...
	  				if(!jQuery.isEmptyObject(res.students[i].studentsAbsenceDelay.Absences)) {
	  					dynamicListRows +='<span class="sections-main-sub-container-left-card-satuts red-color" data-lang="Absence">Absence</span>';
	  				}else if(!jQuery.isEmptyObject(res.students[i].studentsAbsenceDelay.Retards)) {
	  					dynamicListRows +='<span class="sections-main-sub-container-left-card-satuts blue-color" data-lang="Retard">Retard</span>';
	  				}
			...
			$global_student_list = $('#list_classes').clone();
...
function remove() {
	...
	startSpinner("#ConfirmDeleteModal .sub-container-form-footer");
	...
	$.ajax({
		  ...
		  .done(function(res){
		  	...
		  	{
				setTimeout(()=>{
					stopSpinner("#ConfirmDeleteModal .sub-container-form-footer");
	...
	return new Promise((resolve, reject) => {
		...
		.done(function(res){
			if(res.errors){
				...
				console.log("std current =>",res.substudent);
					
				$("#Finance").find(".sub-container-form-footer").removeClass("hide-footer");
				removeLoadingAnimation($detailsSelector,$headerInfo);
				var checked = '';
				...
				console.log("all Substudent pay => ",res.substudentpay);
				...
				for (var i = indStart; i < months.length; i++) {
					...
					institution_months.push(months[i]);
					...
					for (var k = res.substudentpay.length - 1; k >= 0; k--) {
						...
						if (res.substudentpay[k].Expense_PaymentMethod === "Monthly"){
							...
							if (date.getMonth() >= i || i >=  months.indexOf(res.substudentpay[k].Subscription_StartDate) && endDate >= i){
								...
								if (res.substudentpay[k].SS_Status == "0") {
									
									endDate = months.indexOf(res.substudentpay[k].Subscription_StartDate);
									
									if (institution_months.indexOf(months[i]) <= institution_months.indexOf(res.substudentpay[k].Subscription_EndDate_Month) && endDate <= i ) {
									...
									if( i >=  months.indexOf(res.substudentpay[k].Subscription_StartDate) && i <= date.getMonth() ){
										unpaid += parseInt(res.substudentpay[k].Expense_Cost);
										$("#Finance").find('[data-val="' + res.substudentpay[k].Expense_Label + '"]').append('<td data-id="' + res.substudentpay[k].SS_ID + "-" + months[i] + '" ' + style + ' scope="col" class="row-payment col-text-align "><img  src="assets/icons/check_red.svg" alt="states"/></td>');
										red_check_marks++;
									}else{
				...
				for (var i = res.parents.length - 1; i >= 0; i--) {
					...
					$('#Details').find('.sections-main-sub-container-right-main-rows-parents').prepend('<div class="row-payment dynamic-form-input-parent '+inputFirst+' row-parent"> <div class="input-parent "><div class="col-md-4"> <div class="form-group group "> <input type="text" required="" data-id="'+res.parents[i].Parent_ID+'" name="parent_name" value="'+res.parents[i].Parent_Name+'"  '+readOnly+'> <label class="input-label '+inputLabel+'"> <span class="input-label-text" data-lang="Full_name">Full name</span><span class="input-label-bg-mask"></span> </label> </div> </div><div class="col-md-4"> <div class="form-group group "> <input type="text" required="" data-id="'+res.parents[i].Parent_ID+'" name="parent_phone" value="'+res.parents[i].Parent_Phone+'"  '+readOnly+'> <label class="input-label '+inputLabel+'"> <span class="input-label-text" data-lang="Phone_number">Phone number</span><span class="input-label-bg-mask"></span> </label> </div> </div> <div class="col-md-4"> <div class="form-group group "> <input type="text" required="" data-id="'+res.parents[i].Parent_ID+'" name="parent_email" value="'+res.parents[i].Parent_Email+'"  '+readOnly+'> <label class="input-label '+inputLabel+'"> <span class="input-label-text" data-lang="Email">Email</span><span class="input-label-bg-mask"></span> </label> </div> </div> </div>');

					$('#EditStudentModal').find('.sections-main-sub-container-right-main-rows-parents').prepend('<div class="row-payment dynamic-form-input-parent '+inputFirst+' row-parent"> <div class="input-parent "><div class="col-md-12"><div class="dynamic-form-input-dropdown-container dynamic-form-input-dropdown-search-container"><div class="dynamic-form-input-dropdown dynamic-form-input-first"><div class="dynamic-form-input"><div class="dynamic-form-input-float-adjust"><div class="form-group group form-group-right" style="margin-bottom: 0 !important;"><input type="text" class="input-dropdown-search" placeholder="'+arrLang[$lang]["Search_parents"]+'" data-lang="Search_parents" name="parent_name" value="'+res.parents[i].Parent_Name+'" data-id="'+res.parents[i].Parent_ID+'" /><img class="icon button-icon caret-rotate" src="assets/icons/sidebar_icons/search.svg"><ul class="dynamic-form-input-dropdown-options"></ul></div></div></div></div></div></div><div class="col-md-6"> <div class="form-group group "> <input type="text" required="" data-id="'+res.parents[i].Parent_ID+'" name="parent_phone" value="'+res.parents[i].Parent_Phone+'"> <label class="input-label"> <span class="input-label-text" data-lang="Phone_number">Phone number</span><span class="input-label-bg-mask"></span> </label> </div> </div> <div class="col-md-5"> <div class="form-group group "> <input type="text" required="" data-id="'+res.parents[i].Parent_ID+'" name="parent_email" value="'+res.parents[i].Parent_Email+'"> <label class="input-label"> <span class="input-label-text" data-lang="Email">Email</span><span class="input-label-bg-mask"></span> </label> </div> </div> <div class="col-md-1"> <div class="square-button"> <img class="icon" src="assets/icons/minus.svg"> </div> </div> </div> </div>');
				}
				...
				if (result[0])
					{
						...
						if (subclasses[i].Classe_Label === result[0].Classe_Label)
						{
							...
							for (var j = res.substudent.length - 1; j >= 0; j--) {
								if(subclasses[i].Expense_Label === res.substudent[j].Expense_Label && res.substudent[j].SS_Status == "1"){
									checked = 'checked';
								}	
							}
							$('#Details').find('.sub_list').append('<div class="expense_col col-md-6 sections-label-checkbox-main-container "> <div class="sections-label-checkbox-container"> <div class="form-group group "> <span class="expense_label">'+subclasses[i].Expense_Label+'</span> <span class="method_label"> <span class="method_label_price">'+subclasses[i].Expense_Cost+'</span> <span class="method_label_period" data-lang="'+subclasses[i].Expense_PaymentMethod+'">'+subclasses[i].Expense_PaymentMethod+'</span> </span> </div> </div> <div class="customCheck readonly"> <input type="checkbox"  value="'+subclasses[i].LE_ID+'" name="checkbox" id="ck" '+checked+'/> <label for="ck"></label> </div> </div> ');
							$('#EditStudentModal').find('.sub_list').append('<div class="expense_col col-md-6 sections-label-checkbox-main-container "> <div class="sections-label-checkbox-container"> <div class="form-group group "> <span class="expense_label">'+subclasses[i].Expense_Label+'</span> <span class="method_label"> <span class="method_label_price">'+subclasses[i].Expense_Cost+'</span> <span class="method_label_period" data-lang="'+subclasses[i].Expense_PaymentMethod+'">'+subclasses[i].Expense_PaymentMethod+'</span> </span> </div> </div> <div class="customCheck "> <input type="checkbox"  value="'+subclasses[i].LE_ID+'" name="checkbox" id="ck" '+checked+'/> <label for="ck"></label> </div> </div> ');
						}
					}
					...
					$('#Details').find('input[name="student_gender_detail"]').val(arrLang[$lang][result[0].Student_Gender]);
					$('#Details').find('input[name="student_gender_detail"]').attr("data-val",result[0].Student_Gender);
					...
					$('#EditStudentModal').find('input[name="student_gender_detail"]').val(arrLang[$lang][result[0].Student_Gender]);
					$('#EditStudentModal').find('input[name="student_gender_detail"]').attr("data-val",result[0].Student_Gender);
					...
					LoadClasses(result[0].Level_Label);
					...
					$('#student_code').html(result[0].Student_Code);
					$('#EditStudentModal').find('input[name="student_code"]').val(result[0].Student_Code);
...
function executePayment() {
	...
	for (var i = subStudent.length - 1; i >= 0; i--) {
		...
		if (subStudent[i].Expense_PaymentMethod === "Monthly")
		{
			...
			for (var j = months.indexOf(subStudent[i].Subscription_StartDate); j < months.length; j++) {
				...
				MonthsFiltred_Prov.push(months[j]);
				if (j === indEnd){
					break;
				}
			...
			for (var k = 0; k < MonthsFiltred.length; k++) {
				...
				if(MonthsFiltred[k] == months[currentMonth]){
					selected = 'selected';
				}else if(MonthsFiltred_Prov.indexOf(MonthsFiltred[k]) >= MonthsFiltred_Prov.indexOf(months[currentMonth])){
					console.log("passByMonth else if",MonthsFiltred[k])
					selected = '';
				}

				if(subStudent[i].SS_Status == "1"){
					htmlmonths += '<option ' + selected + ' data-val="' + MonthsFiltred[k] + '" value="' + MonthsFiltred[k] + '" data-lang="' + arrLang[$lang][MonthsFiltred[k]] + '">' + arrLang[$lang][MonthsFiltred[k]] + '</option>';
					option_counter++;

				}else{

					var endDate_ = MonthsFiltred_Prov.indexOf(subStudent[i].Subscription_EndDate_Month);
					var currentMonth = MonthsFiltred_Prov.indexOf(months[date.getMonth()]);
					var unpaidMonthPosistion = MonthsFiltred_Prov.indexOf(MonthsFiltred[k]);
					
					if( unpaidMonthPosistion <= endDate_ ){

						if( unpaidMonthPosistion <= currentMonth ){
							selected = 'selected';
							htmlmonths += '<option ' + selected + ' data-val="' + MonthsFiltred[k] + '" value="' + MonthsFiltred[k] + '" data-lang="' + arrLang[$lang][MonthsFiltred[k]] + '">' + arrLang[$lang][MonthsFiltred[k]] + '</option>';
							option_counter++;
						}
	...
	console.log(option_counter++);
	return option_counter++;;


}

function executePaymentBtnToggle() {
...
function displayHomework(id) {
	...
	$('#HomeworkDetailModal').find('.sub-container-sub-header').html(''+homeworks[id].Subject_Label+' - '+homeworks[id].Classe_Label);
...
function displayExam(id) {
	console.log("exams[id] ",exams[id]);
	$('#ExamDetailModal').find('.sub-container-main-header').html(exams[id].Exam_Title);
	$('#ExamDetailModal').find('.sub-container-sub-header').html(''+exams[id].Subject_Label+' - '+exams[id].Classe_Label);
		  ...
		  .done(function(res){
		  	if(res.errors)
		  	{
		  		...
		  		for (var i = res.subscriptions.length - 1; i >= 0; i--) {
		  			...
		  			$('#list_sub').append('<div class="col-md-6 sections-label-checkbox-main-container sub_div"> <div class="sections-label-checkbox-container"> <div class="form-group group "> <span class="expense_label">'+res.subscriptions[i].Expense_Label+'</span> <span class="method_label"> <span class="method_label_price">'+res.subscriptions[i].Expense_Cost+'</span> <span class="method_label_period" data-lang="'+res.subscriptions[i].Expense_PaymentMethod+'">'+res.subscriptions[i].Expense_PaymentMethod+'</span> </span> </div> </div> <div class="customCheck"> <input type="checkbox" value="" name="checkbox_sub_'+i+'" id="ck'+i+'" /> <label for="ck'+i+'"></label> </div> </div>')
		  		}
...
$(document).find('#EditStudentModal input[name="classe-detail"]').on("click", function () {
	$('#EditStudentModal .sub-container-form-footer').removeClass('hide-footer');
	$('#EditStudentModal .modal-content').css('height',"calc(100% - 72px)");
	$('#EditStudentModal .sub-container-form-footer').removeClass('hide-footer');
});

$(document).find('#EditStudentModal .input_level .dynamic-form-input-dropdown-options li').on("click", function () {
			  	...
			  	for (var i = 0; i < res.subscriptions.length; i++) {
					...
					if(value == res.studentLevel[0].Level_Label){ 
						for (var j = subStudent.length - 1; j >= 0; j--) {
							if(res.subscriptions[i].Expense_Label === subStudent[j].Expense_Label && subStudent[j].SS_Status == "1" ){
								checked = 'checked';
							}
						}
					}

					$('#EditStudentModal').find('.sub_list').append('<div class="expense_col col-md-6 sections-label-checkbox-main-container "> <div class="sections-label-checkbox-container"> <div class="form-group group "> <span class="expense_label">'+res.subscriptions[i].Expense_Label+'</span> <span class="method_label"> <span class="method_label_price">'+res.subscriptions[i].Expense_Cost+'</span> <span class="method_label_period" data-lang="'+res.subscriptions[i].Expense_PaymentMethod+'">'+res.subscriptions[i].Expense_PaymentMethod+'</span> </span> </div> </div> <div class="customCheck"> <input type="checkbox" value="'+res.subscriptions[i].LE_ID+'" name="checkbox" id="ck" '+checked+'/> <label for="ck"></label> </div> </div> ');
				}
...
function saveStudent() {
	...
	var student_gender = $('#student_form').find('input[name="student_gender"]').attr("data-val");
	...
	var student_code = $('#student_form').find('input[name="student_code"]').val();

	var valide_student_phone = true;
	var valide_parents_phone = [];
	...
	if (!student_code){
		$('#student_form').find('input[name="student_code"]').parent(".form-group").addClass("form-input-error");
	}
	else{
		$('#student_form').find('input[name="student_code"]').parent(".form-group").removeClass("form-input-error");
	}
	...
	if (first_name && last_name && level && classe && parent_phone.length >= 0 && parent_email.length > 0  && parent_name.length > 0 && parent_errors.length == 0 && valide_parents_phone.length == 0 && student_address && valide_student_phone && student_email && emailValidator(student_email) && student_gender && birthdate && checkbox_sub.length > 0 && student_code )
	{
		startSpinner("#student_form .sub-container-form-footer");
		var data = {
			first_name,
			last_name,
			level,
		...
		$.ajax({
		  ...
		  .done(function(res){
		  	...
		  	{
				setTimeout(()=>{
					stopSpinner("#student_form .sub-container-form-footer");
					...
					$('#student_form').find('input[name="classe"]').val("");
					$('#student_form').find('input[name="student_code"]').val("");
					...
					if (student_code == res.form_errors.Student.Code ){
						$('#student_form').find('input[name="student_code"]').parent(".form-group").addClass("form-input-error");
					}
					else{
						$('#student_form').find('input[name="student_code"]').parent(".form-group").removeClass("form-input-error");
...
function saveChange() {
	...
	var student_gender = $('#EditStudentModal').find('input[name="student_gender_detail"]').attr("data-val");
	...
	var student_code = $('#EditStudentModal').find('input[name="student_code"]').val();
	...
	if (!student_code){
		$('#EditStudentModal').find('input[name="student_code"]').parent(".form-group").addClass("form-input-error");
	}
	else{
		$('#EditStudentModal').find('input[name="student_code"]').parent(".form-group").removeClass("form-input-error");
	}
	...
	if (first_name && level && classe && parent_phone.length >= 0 && parent_email.length > 0  && parent_name.length > 0 && parent_errors.length == 0 &&valide_parents_phone.length == 0 && student_address && valide_student_phone && student_email && emailValidator(student_email) && student_gender && birthdate && checkbox_sub.length > 0 && student_code)
	{

	startSpinner("#EditStudentModal .sub-container-form-footer");
	...
	$.ajax({
	    ...
	    data: {
			...
			student_gender:$('#EditStudentModal').find('input[name="student_gender_detail"]').attr("data-val"),
			...
			student_code:$('#EditStudentModal').find('input[name="student_code"]').val()
	    },
	    dataType: 'json'
	  })
	  .done(function(res){
	  	...
	  	if(res.updated) {
			...
			setTimeout(()=>{
				stopSpinner("#EditStudentModal .sub-container-form-footer");
...
function updateAttitude() {
	...
	if(at_note && at_date){
		startSpinner("#EditAttitudeModal .sub-container-form-footer");
		$.ajax({
	    	type: 'post',
		  ...
		  .done(function(res){
		  	...
		  	{
				setTimeout(()=>{
					stopSpinner("#EditAttitudeModal .sub-container-form-footer");
...
function saveAbsence() {
	...
	{
		startSpinner("#AddStudentAbsenceModal .sub-container-form-footer");
		$.ajax({
		    type: 'post',
		  ...
		  .done(function(res){
		  	if(res.saved)
		  	{
				...
				setTimeout(()=>{
					stopSpinner("#AddStudentAbsenceModal .sub-container-form-footer");
					$('.input-time').timepicker('destroy');
...
function saveAttitude() {
	...
	{
		startSpinner("#AddAttitudeModal .sub-container-form-footer");
		$.ajax({
		    type: 'post',
		  ...
		  .done(function(res){
		  	...
		  	{
				setTimeout(()=>{
					stopSpinner("#AddAttitudeModal .sub-container-form-footer");
...
$(document).on("click",".dynamic-form-input-dropdown-options-search li",function(event){
		...
		/****************************************************/
			...
			SortStudentList(students);
			filtredClass = students;
			
			if (students.length > 0){
				displayStudent(students[students.length - 1].Student_ID);
			}
			...
			for (var i = students.length - 1; i >= 0; i--) {
					...
					if(!jQuery.isEmptyObject(students[i].studentsAbsenceDelay.Absences)) {
						dynamicListRows +='data-absence="1" ';
					}else if(!jQuery.isEmptyObject(students[i].studentsAbsenceDelay.Retards)) {
						dynamicListRows +='data-retard="1" ';
					}

					dynamicListRows +='class="'+active+' sections-main-sub-container-left-card students_list"><img class="sections-main-sub-container-left-card-main-img" src="'+students[i].Student_Image+'" alt="card-img"><span class="sections-main-sub-container-left-card-main-img-text loading-bg-helper"></span><input name="studentId" type="hidden" value="'+students[i].Student_ID+'"> <div class="sections-main-sub-container-left-card-info"><p class="sections-main-sub-container-left-card-main-info">'+students[i].Student_FirstName+' '+students[i].Student_LastName+'</p><span  class="sections-main-sub-container-left-card-sub-info">'+students[i].Classe_Label+'</span></div>';
		...
		if($(".students_list").length == $(".students_list.hidden").length ){
			$HeaderFeedBack = "No result found !";
			$SubHeaderFeedBack = "";
			$IconFeedBack = "404_students.png";
			no_Result_FeedBack($HeaderFeedBack,$SubHeaderFeedBack,$IconFeedBack);
