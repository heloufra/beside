  ...
  $.ajax({
    ...
    if (res.errors) {
      ...
      if (res.exams.length > 0) {
        ...
        if (id) displayExam(id);
        else displayExam(res.exams[0].Exam_ID);
      }
      var active = "";
      var name = "";
      ...
      for (var i = 0; i <= res.exams.length - 1; i++) {
        ...
        dynamicListRows +=
          ...
          ';" >' +
          res.exams[i].Subject_Label.slice(0, 2) +
          '</div> <div class="sections-main-sub-container-left-card-info"> <p class="sections-main-sub-container-left-card-main-info">' +
          res.exams[i].Exam_Title +
          '</p> <span class="sections-main-sub-container-left-card-sub-info">' +
          res.exams[i].Subject_Label +
          " - " +
          res.exams[i].Classe_Label +
          " - " +
          name.first_name +
          " " +
          name.last_name +
...
function saveExam() {
  ...
  var exam_subject = $("#AddExamModal")
    .find('input[name="exam_subject"]')
    .val();
  ...
  else $("#AddExamModal").find(".exam_date").css("border-color", "#EFEFEF");
  if (!exam_classe)
    $("#AddExamModal").find(".exam_classe").css("border-color", "#f6b8c1");
  else $("#AddExamModal").find(".exam_classe").css("border-color", "#EFEFEF");
  if (!exam_name)
    $("#AddExamModal").find(".exam_name").css("border-color", "#f6b8c1");
  else $("#AddExamModal").find(".exam_name").css("border-color", "#EFEFEF");

  if (!exam_subject)
    $("#AddExamModal").find(".exam_subject").css("border-color", "#f6b8c1");
  else $("#AddExamModal").find(".exam_subject").css("border-color", "#EFEFEF");
  if (!exam_description)
...
function displayExam(index) {
  ...
  $.ajax({
    ...
    if (res.errors) {
      ...
      if (res.exam[0]) {
        var readonly = "";
        if (res.role === "Admin") readonly = "readonly";

        for (var i = res.score.length - 1; i >= 0; i--) {
          $(".scores-container").append(
            '<tr class="row-score"> <td data-label="Exam Title"> <!-- sections-main-sub-container-left-cards --> <div class="sections-main-sub-container-left-card"> <img class="sections-main-sub-container-left-card-main-img" src="' +
              res.score[i].Student_Image +
              '" alt="card-img"> <div class="sections-main-sub-container-left-card-info"> <p class="sections-main-sub-container-left-card-main-info">' +
              res.score[i].Student_FirstName +
              " " +
              res.score[i].Student_LastName +
              '</p> <span class="sections-main-sub-container-left-card-sub-info">' +
              res.score[i].Classe_Label +
              '</span> </div> </div> <!-- End sections-main-sub-container-left-cards --> </td> <td data-label="Score"> <div class="form-group group dynamic-form-input-text-container-icon"> <input ' +
              readonly +
              ' type="number" data-gradid="' +
              res.score[i].Grad_ID +
              '" data-studentid="' +
              res.score[i].Student_ID +
              '" name="score" value="' +
        ...
        $("#exam_info")
          .find(".exam_img")
          .css("background-color", res.exam[0].Subject_Color);
        if (res.average.average)
          $("#exam_info").find("#avg").html(res.average.average.toFixed(2));
        else $("#exam_info").find("#avg").html("0.00");
        $("#exam_info")
          .find(".exam_img")
          .html(res.exam[0].Subject_Label.slice(0, 2));
        $("#exam_info").find(".label-full-name").html(res.exam[0].Exam_Title);
        $("#exam_info")
          .find('input[name="exam_name"]')
          .val(res.exam[0].Exam_Title);
        $("#exam_info")
          .find('input[name="exam_date"]')
          .val(res.exam[0].Exam_Date);
        ...
        $("#EditExamModal")
          ...
          .val(res.exam[0].Exam_Date);
        $("#EditExamModal")
          .find("#exam_description")
          .val(res.exam[0].Exam_Deatils);
        var name = JSON.parse(res.exam[0].User_Name);
        $("#exam_info")
          .find(".sub-label-full-name")
          .html(
            res.exam[0].Subject_Label +
              " - " +
              res.exam[0].Classe_Label +
              " - " +
              name.first_name +
              " " +
              name.last_name
  ...
  var scores = $("input[name^=score]")
    .map(function (idx, elem) {
      if ($(elem).val() >= 0 && $(elem).val() <= 20) {
        ...
        return {
          score: $(elem).val(),
          student: $(elem).data("studentid"),
          gradid: $(elem).data("gradid"),
        };
      } else {
...
$(".exam-filters").on("change", function () {
  var subjectVal = $(
    '.sections-main-sub-container-left-search-bar  input[name="filter-subject"]'
  ).attr("data-val");
  var classeVal = $(
    '.sections-main-sub-container-left-search-bar  input[name="filter-classe"]'
  ).attr("data-val");
  ...
  if (value.replace(/\s/g, "") !== "") {
    ...
    if (classeVal == "All" && subjectVal == "All") {
      ...
      if (classeVal != "All" && subjectVal != "All") {
        examsPrev = exams.filter(
          (ex) => ex.Classe_Label == classeVal && ex.Subject_Label == subjectVal
        );
    ...
    for (var i = examsPrev.length - 1; i >= 0; i--) {
      ...
      dynamicListRows +=
        '<div data-id="' +
        examsPrev[i].Exam_ID +
        '" class="sections-main-sub-container-left-card homework-row ' +
        active +
        '"><input name="homeworkId" type="hidden" value="' +
        examsPrev[i].Exam_ID +
        '"> <div class="sections-main-sub-container-left-card-main-img-text" data-style="background: ' +
        examsPrev[i].Subject_Color +
        ';" >' +
        examsPrev[i].Subject_Label.slice(0, 2) +
        '</div> <div class="sections-main-sub-container-left-card-info"> <p class="sections-main-sub-container-left-card-main-info">' +
        examsPrev[i].Exam_Title +
        '</p> <span class="sections-main-sub-container-left-card-sub-info">' +
        examsPrev[i].Subject_Label +
        " - " +
        examsPrev[i].Classe_Label +
        " - " +
        name.first_name +
        " " +
        name.last_name +
...
$(document).on(
  "click",
  ".sections-main-sub-container-right-main-header-option-list-li-exam-edit",
  function () {
